		...
		methods: {
			...
			async handClick(id) {
				...
				await that.$api.HHPlatForm_P_GetCustomerInfoByID(param).then(res => {
					// debugger
					if (CusType == '1') {
						that.$store.state.test.param.entry1.customer_id = id
						that.$store.state.test.param.entry1.entrust = 0
						that.$store.state.test.param.entry1.bd_name = res[0].BD_NAME
						that.$store.state.test.param.entry1.com_phone = res[0].COM_PHONE
						that.$store.state.test.param.entry1.cer_id_no = res[0].CER_ID_NO
						that.$store.state.test.param.entry2.name = ""
						that.$store.state.test.param.entry2.phone = ""
						that.$store.state.test.param.entry2.identity = ""
						that.$store.state.test.param.entry2.relation = 21
						
						that.$store.state.test.param.entry1.entrust_name = ""
						that.$store.state.test.param.entry1.entrust_phone = ""
						that.$store.state.test.param.entry1.entrust_relation = 21
						that.$store.state.test.param.entry1.entrust_relation_txt = ""
						that.$store.state.test.param.entry1.unit_name =  ""
						that.$store.state.test.param.entry1.regist_no =  ""
						that.$store.state.test.param.entry1.register_site = ""
					} else {
						that.$store.state.test.param.entry1.customer_id = id
						that.$store.state.test.param.entry1.entrust = 1
						that.$store.state.test.param.entry1.entrust_name = ""
						that.$store.state.test.param.entry1.entrust_phone = ""
						that.$store.state.test.param.entry1.entrust_relation = 21
						that.$store.state.test.param.entry1.entrust_relation_txt = ""
						that.$store.state.test.param.entry1.unit_name = res[0].UNIT_NAME
						that.$store.state.test.param.entry1.regist_no = res[0].REGIST_NO
						that.$store.state.test.param.entry1.register_site = res[0].REGISTER_SITE
						
						that.$store.state.test.param.entry1.bd_name = ""
						that.$store.state.test.param.entry1.com_phone = ""
						that.$store.state.test.param.entry1.cer_id_no = ""
						that.$store.state.test.param.entry2.name = ""
						that.$store.state.test.param.entry2.phone = ""
						that.$store.state.test.param.entry2.identity = ""
						that.$store.state.test.param.entry2.relation = 21
